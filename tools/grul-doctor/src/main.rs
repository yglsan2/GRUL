...
use drivers::{run_install, DriverInstallOptions};

use repair::{run_repair, RepairOptions};
...
enum Commands {
    ...
    /// Configure une VM (profil, guest agent, sécurité auto)
...
enum DriversAction {
    ...
    /// Installe les guest tools pour l'hyperviseur détecté
...
fn run() -> Result<(), String> {
    ...
    match cli.command {
        ...
        Some(Commands::Drivers { action }) => match action {

            None | Some(DriversAction::Install) => {

                for line in run_install(&DriverInstallOptions {
        ...
        Some(Commands::Repair { vacuum_journal }) => {

            for line in run_repair(&RepairOptions {
...
fn print_footer() {
    ...
    if vm.is_virtual {
        ...
        println!("  sudo grul vm optimize");

        println!("  sudo grul update -y");

        println!("  sudo grul uninstall");

    } else {

        println!("Prochaines étapes :");

        println!("  sudo grul optimize");
